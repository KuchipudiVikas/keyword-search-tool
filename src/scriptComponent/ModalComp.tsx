...
import { Modal, Button, Divider, Space, Tag, Switch } from "antd";
...
const styleTagSearchResult = {
  cursor: "pointer",
  width: "100%",
  marginBottom: "10px",
};

export const ModalComponet: React.FC<ModalProps> = ({
  isModalOpen,
  setIsModalOpen,
  domain,
}) => {
  ...
  const [isExtendedResults, setIsExtendedResults] = useState(false);
  const handleOk = () => {
  ...
  useEffect(() => {
    if (domain === GOOGLE) {
      setIsExtendedResults(true);
    }
  }, []);

  const onChange = (checked: boolean) => {
    setIsExtendedResults(checked);
  };
    ...
    <>
      ...
      >
        {/* <Divider orientation="left">
            ...
            {Boolean(keywords.length) && (
              <>
                ...
                <Button
                  onClick={() => {
                    copyKeywordsToClipboard(keywords);
                  }}
                  icon={<CopyOutlined />}
                ></Button>
              </>
            ...
            <Divider orientation="left">
              ...
              {Boolean(keywords.length) && (
                <>
                  <Button
                    onClick={() => {
                      downloadCSV(keywords, results, domain);
                    }}
                    icon={<DownloadOutlined />}
                  ></Button>{" "}
                  &nbsp;&nbsp;
                ...
                <div key={colIndex} className="tag-column">
                  {column.map((keyword, tagIndex) => (
                    <div key={`${colIndex}-${tagIndex}`}>
                      ...
                      {results[keyword] && (
                        <Tag color="green" style={styleTagSearchResult}>
        ...
        <SearchComp
          ...
          domain={domain}
          isExtendedResults={isExtendedResults}
        />
      </Modal>
    </>
  );
