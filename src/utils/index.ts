export const downloadCSV = (keywords: string[], results: object): void => {
    // Add the header and a blank line to the keywords array
    const csvContent = ['Etsy Keywords', ''].concat(keywords);

    const finalCsvContent = csvContent.map((key) => {
        if (results[key]) {
...
export const copyKeywordsToClipboard = async (keywords: string[]): Promise<void> => {
